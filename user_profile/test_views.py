...
class TestProfileViews(TestCase):
    ...
    def test_user_profile_returns_200(self):
        ...
        response = self.client.get('/user_profile/user_profile', follow=True)
    ...
    def test_profile_view_dispatch(self):
        ...
        self.client.post(
            f'/user_settings/', {'first_name': 'Test First Name'}
        )
        profile = Profile.objects.get(first_name='Test First Name')
        self.assertEqual(profile.first_name, 'Test First Name')
        response = self.client.get('/user_profile/user_profile/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'user_profile.html')

    def test_profile_view_post(self):
        """
        Tests POST Profile View
        """
        user = self.client.login(
            username='test_user', password='test_password'
        )
        ...
        self.client.post(
            f'/user_settings/', {'bio': 'Test Bio'}
        )
        self.assertEqual(profile.bio, 'Test Bio')
        response = self.client.get('/user_profile/user_settings/')
        self.assertEqual(response.status_code, 200)
